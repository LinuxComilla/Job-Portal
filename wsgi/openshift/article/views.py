...
def register(request):
	...
	msg = EmailMultiAlternatives('Registration', text_content, 'Sheesh@rragroups.com', [email])
...
def profile(request):
	...
	try:
	    ...
	    profile =[]
	    resum = []
        return render_to_response('profile.html', {'details': details,'encoded':b, 'email': request.session['emailid'], 'profile':profile, 'resum':resum})
...
def profilec(request):
	...
	try:
	    ...
	    profile =[]
	    resume = []
        return render_to_response('profile.html', {'details': details,'encoded':b, 'email': request.session['emailid'], 'profile':profile, 'resum':resum,'condition':condition})
...
def deleteaccount(request, another='a'):
    ...
    a.delete()
    a = Resume.objects.get(email = email)
    a.delete()
    condition = "Thanks for reporting, The account has been successfully deleted"
    return render_to_response('Signup.html',{'cond':condition},context_instance=RequestContext(request))

def verifymail(request, another):
    condition="Successfully verified your email address"
    a='out'
    b = base64.b64decode(another)
    email = b.decode('zip')
...
def verifymailid(request, another):
    a='out'
    condition = 'An activation link has been sent'
    conditio = 'Login first to verify your mail'
    if request.session['emailid']==a:
        return render_to_response('Signup.html',{'cond':conditio},context_instance=RequestContext(request))
    else:
	plaintext = get_template('verification.txt')
	htmly = get_template('verification.html')
	email = request.session['emailid']
	d = Context({'encoded':another})
	text_content = plaintext.render(d)
	html_content = htmly.render(d)
	msg = EmailMultiAlternatives('Registration', text_content, 'Sheesh@rragroups.com', [email])
	msg.attach_alternative(html_content, "text/html")
...
def rregister(request):
	...
	plaintext = get_template('ractivation.txt')
	htmly = get_template('ractivation.html')
	a = email.encode('zip')
        b = base64.b64encode(a)
	d = Context({ 'email': email ,'encoded':b})
	text_content = plaintext.render(d)
...
def post_job(request):
    ...
    qualification = request.POST.get('qualification', '')
    p = Postjob(email = email, title = title, description = description, industry = industry, functional = functional, minexp = minexp, maxexp = maxexp, minsalary = minsalary, maxsalary = maxsalary, location = location, jtype = jtype, qualification = qualification)
    p.save()
...
def rdeleteaccount(request, another='a'):
    b = base64.b64decode(another)
    email = b.decode('zip')
    a = Arctic.objects.get(email = email)
    a.delete()
    a = Postjob.objects.filter(email = email)
    a.delete()
    condition = "Thanks for reporting, The account has been successfully deleted"
    return render_to_response('recruiter_signup.html', {'condition':condition}, context_instance=RequestContext(request))

def rverifymail(request, another='a'):
    condition="Successfully verified your email address"
    a='out'
    b = base64.b64decode(another)
    email = b.decode('zip')
	...
	if request.session['remailid']==a:
	    ...
	    request.session['remailid'] = email
    	    details = Arctic.objects.get(email = request.session['remailid'])
    	    return render_to_response('rprofile.html', {'email': request.session['remailid'], 'details':details, 'condition':condition})
    else:
	return render_to_response('recruiter_signup.html', {'condition':condition}, context_instance=RequestContext(request))
...
def editjob(request, jobid):
    ...
    if 'remailid' in request.session:
	if request.session['remailid']!=a:
	    a = Postjob.objects.get(id = jobid)
	    return render_to_response('editjob.html', {'profile':a, 'email':request.session['remailid']}, context_instance=RequestContext(request))
	else:
	    return HttpResponseRedirect('/rsigninerror/')
    else:
	return HttpResponseRedirect('/rsigninerror/')
    
def savechangedjob(request, jobid):
    email = request.session['remailid']
    title = request.POST.get('title', '')
    description = request.POST.get('description', '')
    industry = request.POST.get('industry', '')
    ...
    jtype = request.POST.get('type', '')
    qualification = request.POST.get('qualification', '')
    b = Postjob.objects.filter(id = jobid)
    a = 'out'
    if 'remailid' in request.session:
	if request.session['remailid']==a:
	    return HttpResponseRedirect('/rsigninerror/')
	else:
	    b.update(email = email, title = title, description = description, industry = industry, functional = functional, minexp = minexp, maxexp = maxexp, minsalary = minsalary, maxsalary = maxsalary, location = location, jtype = jtype, qualification = qualification)
	    jobs = Postjob.objects.all().filter(email = request.session['remailid']).order_by('-id')
	    details = Arctic.objects.get(email = request.session['remailid'])
	    condition = "Successfully saved the changes"
            context = {'jobs':jobs, 'email' : request.session['remailid'], 'details':details, 'condition':condition}
            return render_to_response('jobdetails.html', context)
    else:
	return HttpResponseRedirect('/rsigninerror/')
...
def findjob(request):
	...
	c=[]
        ...
        b = a.all().filter(title__icontains = title)
        if not experience:
	    experience=30
...
def appliedlist(request, profileid=1):
    a = 'out'
    b = []
    if 'emailid' in request.session:
	if request.session['emailid']!=a:
	    joblist = Joblist.objects.filter(profileid = profileid)
	    for job in joblist:
		c = Postjob.objects.get(id = job.jobid)
		b.append(c)
	    return render_to_response('appliedjobs.html', {'jobs':b, 'email':request.session['emailid']})
	else:
	    condition = "Login First to view the jobs"
	    request.session['remailid']=a
	    return render_to_response('Signup.html',{'cond':condition},context_instance=RequestContext(request))
    else:
	condition = "Login First to view the job"
	request.session['remailid']=a
	return render_to_response('Signup.html',{'cond':condition},context_instance=RequestContext(request))

def jobget(request, articleid = 1):
    job = Postjob.objects.get(id=articleid)
    details = Arctic.objects.get(email = job.email)
    return render_to_response('jobget.html', {'job': job, 'email':request.session['emailid'], 'details': details })
#####################################################################################################################################################
...
def resumedetails(request, candidateid):
    a = 'out'
    if 'remailid' in request.session:
	if request.session['remailid']!=a:
    	    candidate = Candidate.objects.get(id = candidateid)
	    details = Article.objects.get(email = candidate.email)
	    return render_to_response('resume.html', {'candidate':candidate, 'email':request.session['remailid'], 'details':details})
	else:
    	    return HttpResponseRedirect('/rsigninerror/')
    else:
	return HttpResponseRedirect('/rsigninerror/')

def companycontact(request, another):
    company = Arctic.objects.get(email = request.session['remailid'])
    plaintext = get_template('contactcandidate.txt')
    htmly = get_template('contactcandidate.html')
    d = Context({'company':company})
    text_content = plaintext.render(d)
    html_content = htmly.render(d)
    msg = EmailMultiAlternatives('Job Offer', text_content, 'Sheesh@rragroups.com', [another])
    msg.attach_alternative(html_content, "text/html")
    msg.send()
    condition = "Successfully contacted the student"
    return render_to_response('rprofile.html', {'email': request.session['remailid'], 'details':company, 'condition':condition})
####################################################         For Jobseeker Login        ############################################################
...
def sendmail(request):
	...
	a = email.encode('zip')
        b = base64.b64encode(a)
        send_mail("Job Portal-Reset Your Password","Click on the link to reset your password--  http://jobportal-rragroups.rhcloud.com/resetpassword/%s/" % b, 'Sheesh@rragroups.com', [email], fail_silently=False)
...
def rsendmail(request):
	...
	a = email.encode('zip')
        b = base64.b64encode(a)
        plaintext = get_template('resetpassword.txt')
        htmly = get_template('resetpassword.html')
        d = Context({'encoded':b})
        text_content = plaintext.render(d)
        html_content = htmly.render(d)
        msg = EmailMultiAlternatives('Reset Your Password', text_content, 'Sheesh@rragroups.com', [email])
        msg.attach_alternative(html_content, "text/html")
        msg.send()
